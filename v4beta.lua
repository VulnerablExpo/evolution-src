getgenv().Evolution = {
    Options = {
        AimingArgument = "UpdateMousePos", -- // For Silent Aim
        Panic = {false, 'P'}, -- // Enabled, Keybind
    },
    Silent = {
        Enabled = false,
    },
...
--[[
	TODO:
		spinny cursor
		global chams
		GOOD SILENT AIM - alysum streamable
		closestpoint + closestpart
		camlock
		mouse lock
...
do
	...
	do
		...
		function Library:Window(Options)
            ...
            if not isfolder("EvolutionV4") then 
                makefolder("EvolutionV4")
            end

            if not isfolder("EvolutionV4/configs") then 
                makefolder("EvolutionV4/configs")
            end

            if not isfile("EvolutionV4/configs/Default.cfg") then 
                writefile("EvolutionV4/configs/Default.cfg","")
            end

			-- Config
			Library:Connection(Save.MouseButton1Down, function()
				writefile("EvolutionV4/configs/Default.cfg", Library:GetConfig())
			...
			Library:Connection(Load.MouseButton1Down, function()
				if isfile("EvolutionV4/configs/Default.cfg") then
					Library:LoadConfig(readfile("EvolutionV4/configs/Default.cfg"))
				end
...
-- // Interface
do
    local EvoUI = Library:Window({User = game.Players.LocalPlayer.DisplayName,Icon = game:GetService("Players"):GetUserThumbnailAsync(game.Players.LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)})
    Library.Watermark:UpdateText("| Evolution V4")
    EvoUI:Catagory({Name = "Aimbot"})

    -- // Legit Bot
    local LegitBot = EvoUI:Page({Name = "Legit Bot", Icon = "rbxassetid://16081386298"})


    -- // Rage Bot
    local RageBot = EvoUI:Page({Name = "Rage Bot"})



    EvoUI:Catagory({Name = "Extra"})
    -- // Settings Tab
    local settingstab = EvoUI:Page({Name = "Settings", Icon = "rbxassetid://7059346373"})

    local SettingsSection = settingstab:Section({Name = "UI Settings", Size = 200})
    local ThemeSection = settingstab:Section({Name = "Theme", Side = "Right", Size = 200,Scroll = true})
    SettingsSection:Toggle({Name = "Typing Effect", Flag = "Typing", Callback = function(s) Library.Typing = s end})
    SettingsSection:Keybind({Name = "Toggle Interface", Default = Enum.KeyCode.RightShift, Mode = "Toggle", Callback = function(s)
        Library:SetOpen(not s)
    end})
    SettingsSection:Toggle({Name = "Key List", Flag = "KeyList", Callback = function(s) Library.KeyList:SetVisible(s) end})
    SettingsSection:Toggle({Name = "Watermark", Flag = "Watermark", Callback = function(s) Library.Watermark:SetVisible(s) end})
    --
    local ThemePickers = {}
    ThemeSection:List({Name = "Theme", Flag = "SelectedTheme", Options = {"Default","Red","Purple"}})
    ThemeSection:Button({Name = "Load Theme", Callback = function() 
        Library:LoadTheme(Library.Flags["SelectedTheme"])
        for Option,Picker in next, ThemePickers do
            Picker:Set(Library.Theme[Option])
        end;
    end})
    ThemePickers["Accent"] = ThemeSection:Colorpicker({Name = "Accent", Flag = "LibraryAccent", Default = Library.Theme["Accent"], Callback = function(s) Library:ChangeThemeColor("Accent", s) end})
    ThemePickers["Dark Accent"] = ThemeSection:Colorpicker({Name = "Dark Accent", Flag = "LibraryDarkAccent", Default = Library.Theme["Dark Accent"], Callback = function(s) Library:ChangeThemeColor("Dark Accent", s) end})
    ThemePickers["Text"] = ThemeSection:Colorpicker({Name = "Text", Flag = "LibraryText", Default = Library.Theme["Text"], Callback = function(s) Library:ChangeThemeColor("Text", s) end})
    ThemePickers["Dark Text"] = ThemeSection:Colorpicker({Name = "Dark Text", Flag = "LibraryDarkText", Default = Library.Theme["Dark Text"], Callback = function(s) Library:ChangeThemeColor("Dark Text", s) end})
    ThemePickers["Holder"] = ThemeSection:Colorpicker({Name = "Holder", Flag = "LibraryHolder", Default = Library.Theme["Holder"], Callback = function(s) Library:ChangeThemeColor("Holder", s) end})
    ThemePickers["Top"] = ThemeSection:Colorpicker({Name = "Top", Flag = "LibraryTop", Default = Library.Theme["Top"], Callback = function(s) Library:ChangeThemeColor("Top", s) end})
    ThemePickers["Un-Selected"] = ThemeSection:Colorpicker({Name = "Un-Selected", Flag = "LibraryUnSelected", Default = Library.Theme["Un-Selected"], Callback = function(s) Library:ChangeThemeColor("Un-Selected", s) end})
    ThemePickers["Selected"] = ThemeSection:Colorpicker({Name = "Selected", Flag = "LibrarySelected", Default = Library.Theme["Selected"], Callback = function(s) Library:ChangeThemeColor("Selected", s) end})
    ThemePickers["Outline"] = ThemeSection:Colorpicker({Name = "Outline", Flag = "LibraryOutline", Default = Library.Theme["Outline"], Callback = function(s) Library:ChangeThemeColor("Outline", s) end})
    
    local Configs = settingstab:Section({Name = "Configurations", Size = 200})

    local savedconfigs = {}

    for i,v in pairs(listfiles("EvolutionV4/configs")) do 
        table.insert(savedconfigs,string.split(v,"\\")[2])
    end

    local ConfigList = Configs:List({Name = "Select Config",Options = savedconfigs,Flag = "Selected_Config"})

    Configs:Textbox({Name = "Config Name",Placeholder = "semi legit",Flag = "Config_Name"})

    Configs:Button({Name = "Create Config", Callback = function() 
        writefile("EvolutionV4/configs/"..Flags["Config_Name"]..".cfg", Library:GetConfig())
        savedconfigs = {}
        for i,v in pairs(listfiles("EvolutionV4/configs")) do 
            table.insert(savedconfigs,string.split(v,"\\")[2])
        end
        ConfigList:Refresh(savedconfigs)
    end})
    
    Configs:Button({Name = "Save Config", Callback = function() 
        writefile("EvolutionV4/configs/"..Flags["Selected_Config"], Library:GetConfig())
    end})

    Configs:Button({Name = "Load Config", Callback = function() 
        Library:LoadConfig(readfile("EvolutionV4/configs/"..Flags["Selected_Config"]))
    end})

    SettingsSection:Toggle({Name = "Auto Save", Flag = "Auto_Save", Callback = function(s) 
